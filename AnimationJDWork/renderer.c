...
{
    if (player->x >= 0 && player->x <= 575){
        if (player->posture == 'r')
            plotBitmap64(base, player->x, 100, RightSnowBoarder, height);
        else if (player->posture == 'l')
            plotBitmap64(base, player->x, 100, LeftSnowBoarder, height);
    }
}
